...
import { fetchSemanticScholarJournal } from './semanticScholar.js';
...
export const collectionSources = (withSemanticScholar = false) =>
  ['openalex', 'crossref', ...(withSemanticScholar ? ['semanticscholar'] : [])];
...
export async function collectJournals(config, { journalKey, fromDate, toDate,
  ...
  withSemanticScholar = false,
  clients = { openalex: fetchOpenAlexJournal, crossref: fetchCrossrefJournal, semanticscholar: fetchSemanticScholarJournal },
  onSourceResult, ...requestOptions } = {}) {
  ...
  const discoverySummary = { coverage: 'not_proven_complete',
    ...
    sources: ['crossref', 'openalex', 'semanticscholar'].map(source => {
      const results = sourceResults.filter(result => result.source === source);
      if (!results.length) return { source, status: 'not_enabled', observed_unique_papers: null, raw_count: null };
      const accepted = filterSourceRecords(results.flatMap(result => result.records)).accepted;
      return { source, status: results.every(result => result.ok && result.complete) ? 'success' : 'partial_failure',
        observed_unique_papers: mergePapers(accepted, { firstSeenDate, checkedAt }).papers.length,
        raw_count: results.reduce((sum, result) => sum + result.raw_count, 0) };
    }) };
  ...
  return { ...merged, status, discovery_summary: discoverySummary, excluded: filtered.excluded, notices: filtered.notices,
