...
**[打开经管顶刊论文日历](https://w1121188104w-hue.github.io/paper-daily/)**：公开、免登录、只读，可直接把链接分享给团队。用户确认后，已发布现有数据，正式网址、中文搜索和日期详情验收通过；**每日日程尚未启用**。发布结果见[正式上线记录](docs/public-launch-2026-09-10.md)。
...
185项自动化测试全部通过；Windows与服务器之间的正式数据换行保护已实测。正式历史为3个版本、93个文件，已取回本地并通过完整校验。本次发布流程的测试、校验、上传和部署全部通过；线上492条论文展示数据与本地验收包一致。先前487条数据下的完整浏览器操作和320／390／768／1366像素宽度验收见[本地验收记录](docs/local-release-acceptance-2026-09-09.md)；正式网址上的中文搜索和日期详情复核见[上线记录](docs/public-launch-2026-09-10.md)。
...
`.github/workflows/`中的当前版本只接受手动运行，没有推送触发或每日日程。采集和网站发布开关均已开启；开关允许手动执行，不代表每天自动更新。`deploy/github/*.yml.example`中的日程只是未启用示例。服务器采集不调用AI，也不需要配置翻译密钥；本次上线没有重新采集或翻译，见[运行说明](deploy/github/README.md)。
