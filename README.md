...
**[打开经管顶刊论文日历](https://w1121188104w-hue.github.io/paper-daily/)**：公开、免登录、只读，可直接把链接分享给团队。正式网址、中文搜索和日期详情验收通过。用户已授权开启自动采集：**北京时间每天08:17运行，13:17按需补查，保存后自动更新网站**。现按用户要求接入全库及每日自动翻译，不逐篇校对；启用与运行结果见[自动翻译说明](docs/translation-automation.md)。时间可能受GitHub排队影响。见[运行说明](deploy/github/README.md)及[首次上线记录](docs/public-launch-2026-09-10.md)。
...
`.github/workflows/daily-collect.yml`接受手动和定时运行，回查含当天的最近60天；当天所选19刊和范围已经双源完整成功才跳过，部分失败不会误判为完成。采集之后按独立开关执行自动翻译。`translate-library.yml`手动启动存量翻译；`deploy-pages.yml`只手动发布已保存资料。均不监听推送，不会因保存数据产生循环。采集步骤不调用AI；密钥只提供给单独的翻译步骤。见[运行说明](deploy/github/README.md)。
