...
## 开发中：Master List 与三源独立发现（尚未上线）

已增加兼容旧库的论文总名册、精确发现时间／发表月份、逐字段自动待办，以及可选的 Semantic Scholar 独立采集。两轮搜索已接入开发版的版本保存：第一轮补官方清单及漏收论文，第二轮补缺失字段并持久化自动重试记录；347 项离线测试通过，尚未切换生产每日工作流。经用户授权，智谱 Pro 真实搜索和 SerpAPI 免费账户检查已通过；这不代表完整流水线已上线。现有线上双源采集、UI和DeepSeek继续保持。最新状态见[两轮搜索开发与验证记录](docs/search-rollout-2026-09-13.md)，早期实现与只读命令见[Master List开发记录](docs/master-list-development.md)。

## 经管顶刊追踪：公开阅读网站已上线（2026-09-10）

新增：最近60天官方清单滚动核对、来源可追溯的真实英文摘要补全和定期重试。官网访问受限、清单不完整及日期不确定会明确标注，不声称已经全部覆盖。新摘要随后进入原来的自动翻译流程。功能、验收及限制见[官网核对与摘要补全说明](docs/official-enrichment.md)。历史阶段的数量保留在下文，最新数量以网站和运行日志为准。

**[打开经管顶刊论文日历](https://w1121188104w-hue.github.io/paper-daily/)**：公开、免登录、只读，可分享给团队。自动采集目标为**北京时间每天08:17、13:17按需补查，保存后自动更新网站**。现采用每10分钟轻量时间检查和早午时段防重复闸门，缓解单次触发延迟，不是每10分钟采集；GitHub仍不保证准点。见[时间修补说明](docs/schedule-fix-2026-09-12.md)。全库及每日自动翻译不逐篇校对，见[自动翻译说明](docs/translation-automation.md)。另见[运行说明](deploy/github/README.md)及[首次上线记录](docs/public-launch-2026-09-10.md)。
...
全库首轮自动翻译已运行并上线，每日自动翻译开关已核实开启。共处理463篇，新增461篇中文标题、298篇中文摘要；网站现有493条记录、475篇中文标题、311篇完整中文。22篇的24个异常字段暂缓，另16条非研究分类不处理；没有尚未请求的研究候选任务。原文、作者、日期及原有14篇中文保持不变。220项离线测试通过，见[自动翻译结果与使用说明](docs/translation-automation.md)。新流程不逐篇校对，只做基本检查，不补写缺失摘要；密钥仅存于GitHub Actions Secret。之前的人工小批试译属于[历史记录](docs/deepseek-pilot-2026-09-10.md)，不再作为日常流程。
...
启用自动采集时的186项自动化测试全部通过，含全19刊上午成功后下午跳过、次日重新采集的日程验证；Windows与服务器之间的正式数据换行保护已实测。首次上线前正式历史3个版本、93个文件已取回本地并通过完整校验。首次发布的测试、校验、上传和部署全部通过，线上492条论文展示数据与本地验收包一致。先前487条数据下的完整浏览器操作和320／390／768／1366像素宽度验收见[本地验收记录](docs/local-release-acceptance-2026-09-09.md)；正式网址上的中文搜索和日期详情复核见[上线记录](docs/public-launch-2026-09-10.md)。
...
`.github/workflows/daily-collect.yml`接受手动和定时运行，回查含当天的最近60天。轻量闸门每个北京时间时段最多自动放行一次；放行后的采集命令仍只在当天19刊双源完整成功且范围覆盖时跳过。部分失败不冒充成功，也不会因告警每10分钟重复采集。采集之后按独立开关执行自动翻译。`translate-library.yml`手动启动存量翻译；`deploy-pages.yml`只手动发布已保存资料。均不监听推送，不会因保存数据产生循环。采集步骤不调用AI；密钥只给单独的翻译步骤。见[运行说明](deploy/github/README.md)。
