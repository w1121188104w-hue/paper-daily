...
此前DeepSeek手动小批试译已完成并上线：10次请求无重试，新增9篇中文标题和其中8篇完整摘要，另一篇继续待翻译。自动翻译前基线为493条记录、14篇中文标题、13篇完整中文；历史结果见[试译与上线核对](docs/deepseek-pilot-2026-09-10.md)。用户随后取消逐篇校对，授权全库和每日自动翻译。新流程直接保存通过基本格式、中文及数字检查的机器译文，保留已有中文，不补写缺失摘要。密钥仅存于GitHub Actions Secret。219项离线测试通过，含远端预登记、防止重复调用及真实Git提交测试。
