...
已增加兼容旧库的论文总名册、精确发现时间／发表月份、逐字段自动待办，以及可选的 Semantic Scholar 独立采集。期刊清单搜索与单篇缺失字段搜索、固定兜底顺序和额度保护已完成离线测试，尚未接入正式搜索修复和每日工作流。经用户授权正在通过手动任务验证真实搜索接口；这不代表正式流水线已启用。现有线上双源采集、UI和DeepSeek继续保持。最新状态见[两轮搜索开发与验证记录](docs/search-rollout-2026-09-13.md)，早期实现与只读命令见[Master List开发记录](docs/master-list-development.md)。
