...
已增加兼容旧库的论文总名册、精确发现时间／发表月份、逐字段自动待办，以及可选的 Semantic Scholar 独立采集。智谱／SerpAPI 搜索适配器、固定兜底顺序和额度保护已完成离线测试，尚未接入正式搜索修复和每日工作流，也未启用收费请求。现有线上双源采集、UI和DeepSeek继续保持。开发进度、只读查看命令及下一步配置见[Master List开发记录](docs/master-list-development.md)。
