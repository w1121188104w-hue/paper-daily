...
新增DeepSeek手动小批试译入口：最多10篇、无自动重试、只上传加密草稿，逐篇审阅后才走原有安全导入和发布流程；不启用全库或每日自动翻译，不补写缺失摘要。密钥存于GitHub Actions Secret，详见[DeepSeek操作说明](docs/deepseek-translation.md)。本地201项离线测试通过，真实API运行结果另行记录。
