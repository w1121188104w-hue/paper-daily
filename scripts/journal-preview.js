...
export async function runPreviewCommand(args = process.argv.slice(2)) {
  const {values} = parseArgs({args,options:{check:{type:'boolean'},port:{type:'string'},library:{type:'string'}}});
  const root = values.library ? path.resolve(values.library) : undefined;
  if (values.check && !values.port) {
    const data = await loadJournalPresentation(await loadJournalConfig(), {root});
  ...
  if (values.check || (values.port && (!/^\d{4,5}$/.test(values.port) || Number(values.port) < 1024 || Number(values.port) > 65535))) throw new Error('参数无效');
  const port = values.port ? Number(values.port) : 4317;
  const server = createJournalPreviewServer({ config: await loadJournalConfig(), root });
