...
**2026-09-12时间修补：** 下文最初启用记录保留作历史。当前日程改为每10分钟轻量检查，按实际北京时间08:17和13:17两个业务时段放行，每个自动时段最多预约一次；不是每10分钟重新采集。手动入口新增 `respect_schedule`（默认关闭）。这能缓解单次定时触发延迟，但不是准点保证。见[原因、保护规则与验收](../../docs/schedule-fix-2026-09-12.md)。
