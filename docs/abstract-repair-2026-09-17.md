...
正式运行 35262268425 的已保存修复报告包含 76 次 HTTP 429 / 业务码 1113。
官方错误码文档将 1113 定义为账户欠费，1302 才是请求速率限制。
旧逻辑只保护同一未知请求，换一个查询仍会调用，导致同轮多次无效请求。
新增本轮智谱停止标记：首次 1113 已按原规则保留未知计费占额，此后不再预占或发送
任何新的智谱请求；其他来源及 SerpAPI 原有免费上限不变。不会自动充值、升级或清零账本。
下一次运行重新允许检查（旧未知查询仍受原冷却限制），以便用户充值后恢复。
不把其他 429 误判为欠费，也不把失败标为“没有摘要”。
来源：[智谱官方错误码](https://docs.bigmodel.cn/cn/faq/api-code)。
