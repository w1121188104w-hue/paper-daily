...
test('工作流先校验再暂存推送，随后构建和发布；不上传整个仓库或论文库', async () => {
  ...
  assert.equal(steps.filter((step) => JSON.stringify(step).includes('secrets.')).length, 3);
...
test('按用户授权允许公开仓库，仍要求明确启用开关且不读取或上传个人配置', async () => {
  for (const [name, jobKey] of [['daily-collect', 'collect'], ['deploy-pages', 'build']]) {
    ...
    assert.equal(secretSteps.length, name === 'daily-collect' ? 3 : 0);
    const conditions = { enrich: "steps.search_gate.outputs.enabled != 'true' && vars.JOURNAL_ENRICHMENT_ENABLED == 'true'",
      pipeline: "steps.search_gate.outputs.enabled == 'true'", translate: "vars.JOURNAL_TRANSLATION_ENABLED == 'true'" };
    for (const step of secretSteps) assert.equal(step.if, conditions[step.id]);
  }
});
